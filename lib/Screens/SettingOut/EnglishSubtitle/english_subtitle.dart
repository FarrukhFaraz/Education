...
class EnglishSubtitle extends StatefulWidget {
  const EnglishSubtitle({Key? key}) : super(key: key);

  @override
  State<EnglishSubtitle> createState() => _EnglishSubtitleState();
}

class _EnglishSubtitleState extends State<EnglishSubtitle> {
  ...
  loadData() async {
    ...
    try {
      http.Response response = await http.get(Uri.parse(englishSubtitleURL),
      ...
      if (jsonData['status'] == 1) {
        setState(() {
          loader = false;
          videoUrl = jsonData['data']['settvideo'];
        });
        print(videoUrl);
